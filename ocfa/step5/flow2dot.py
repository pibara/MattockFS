...
for jsonline in jsonstream:
    ...
    for job in obj["jobs"]:
        ...
        if lastmodule != None:
            ...
            #If the last module was a creator, add an entry to the creator map.
            if lastmodule[-1] == "'":
                creator[arc] = lastmodule
...
#Find those creators that have created at least 0.25% of the data or at least 1% of the events.
mycreators = {}
for arc in arcs:
    ...
    if percentage >= 0.25 or percentage2 >= 1:
        if arc in creator:
            mycreators[creator[arc]] = 1
#Give those creators a box shape.
for creator in mycreators: 
    print '"' + creator + '"  [shape=box];'
...
for arc in arcs:
    ...
    if percentage >= 0.25 or percentage2 >= 1:
        if percentage < 0.25:
            if percentage2 > 4:
                print arc + ' [ color=green ] ;'
            else:
                print arc + ' [ color=green, style=dashed ] ;'
        else:
            if percentage < 1:
                if percentage2 > 4:
                    print arc + ';'
                    ...
                    if percentage2 > 1:
                        print arc + ' [ style=dashed ] ;'
                    else:
                        print arc + ' [ style=dotted ] ;'
            else:
                if percentage2 > 4:
